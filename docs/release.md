...
发给试用的朋友一条消息（推荐直接甩 bootstrap 一行）：
...
已经装过的：再跑同一条命令，原地覆盖升级（bootstrap 会先 ./pdl stop，数据在 ~/ 下不受影响）。

Windows 或离线场景：走 Releases 页下 tarball + 手动 install.sh/install.ps1。
https://github.com/Conner2077/polars-daily-log/releases

主要变化：<changelog 前 2-3 点>
```

> bootstrap.sh 本身是仓库根目录的脚本，tag/master 不冲突 —— 用户走 raw.githubusercontent 拉 **master 分支**的版本（所以 bootstrap 的改动 merge 到 master 就生效，不用重新打 release）。
